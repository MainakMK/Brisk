#!/bin/sh
# Hotlink-protection rollout: rolls the hotlink-capable brisk-agent onto ONE live edge
# over SSH KEY auth. Hotlink ships DARK (no zone has hotlink_enabled), so the new agent
# MUST render a BYTE-IDENTICAL nginx.conf. This script ASSERTS sha256 BEFORE == AFTER and
# exits non-zero if they differ (so the caller halts the rollout). Backs up the old binary
# (.prev-hl) and nginx.conf (.bak-hl) for a one-command rollback.
#
# Usage (alpine + openssh-client; mount tunnels dir at /env):
#   sh /env/deploy-hotlink.sh NY|DE|BLR
# Creds read from /env/.env by prefix and used internally — NEVER echoed.
set -e
PFX="$1"
[ -n "$PFX" ] || { echo "usage: deploy-hotlink.sh NY|DE|BLR"; exit 2; }
...
$SSH "${USER}@${IP}" "
  set -e
  cp -a /usr/local/bin/brisk-agent /usr/local/bin/brisk-agent.prev-hl
  cp -a /etc/nginx/nginx.conf /etc/nginx/nginx.conf.bak-hl
...
$SSH "${USER}@${IP}" "
  ...
  echo hotlink_valid_referers=\$(grep -c 'valid_referers' /etc/nginx/nginx.conf)   # expect 0 = gated off
  echo hotlink_guards=\$(grep -c 'invalid_referer' /etc/nginx/nginx.conf)           # expect 0 = gated off
"

# Hard byte-identical gate: hotlink ships dark, so the rendered nginx.conf MUST be unchanged.
if [ "$BEFORE" != "$AFTER" ]; then
  echo "[$PFX] !!! BYTE-IDENTICAL GATE FAILED: nginx.conf changed ($BEFORE != $AFTER) — ROLL BACK with brisk-agent.prev-hl"
  exit 1
fi
