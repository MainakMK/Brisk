#!/bin/sh
# Tech/runtime rollout: rolls the brisk-agent that reports its PoP's tech stack (OS pretty
# name, kernel release, Go runtime) on the heartbeat, alongside the nginx + agent versions it
# already sent. This change touches ZERO nginx rendering — it only adds fields to the heartbeat
# JSON + two best-effort detect helpers (DetectOSPretty / DetectKernel) — so the new agent MUST
# render a BYTE-IDENTICAL nginx.conf. This script ASSERTS sha256 BEFORE == AFTER and exits
# non-zero if they differ (caller halts). Backs up the old binary (.prev-tech) and nginx.conf
# (.bak-tech) for one-command rollback. nginx is NOT stopped — it keeps serving while the agent
# swaps + re-renders (nginx -t before reload; rollback on failure), so the edge never stops.
#
# Usage (alpine + openssh-client; mount tunnels dir at /env):
#   sh /env/deploy-tech.sh NY|DE|BLR
# Creds read from /env/.env by prefix and used internally — NEVER echoed.
set -e
PFX="$1"
[ -n "$PFX" ] || { echo "usage: deploy-tech.sh NY|DE|BLR"; exit 2; }
...
$SSH "${USER}@${IP}" "
  set -e
  cp -a /usr/local/bin/brisk-agent /usr/local/bin/brisk-agent.prev-tech
  cp -a /etc/nginx/nginx.conf /etc/nginx/nginx.conf.bak-tech
...
# Hard byte-identical gate: heartbeat-only change, no nginx rendering touched, so the rendered
# nginx.conf MUST be unchanged.
if [ "$BEFORE" != "$AFTER" ]; then
  echo "[$PFX] !!! BYTE-IDENTICAL GATE FAILED: nginx.conf changed ($BEFORE != $AFTER) — ROLL BACK with brisk-agent.prev-tech"
  exit 1
fi
